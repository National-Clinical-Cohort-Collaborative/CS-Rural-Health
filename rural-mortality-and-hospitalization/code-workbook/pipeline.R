...
library(rvest)
library(dplyr)
library(tidyverse)@transform_pandas(
...
ROC_data <- function(survival_data_hosp) {
  ...
  survivalROC_data %>%
    ...
    geom_line() +
    geom_label(
      data = survivalROC_data %>% dplyr::select(t, auc) %>% unique,
      mapping = aes(label = sprintf("%.3f", auc)),
      x = 0.5,
      y = 0.5
    ) +
    facet_wrap(~ t) +
...
aa_fit <- function(survival_data_hosp) {
  ...
  local_df <-
    survival_data_hosp %>% dplyr::select(
      three_category_ruca,
      gender,
      age,
      age_Group,
  ...
  local_df <-
    filter(local_df, quarter_of_diagnosis != "Unknown/Missing")
  ...
  local_df$three_category_ruca = relevel(as.factor(local_df$three_category_ruca), ref =
                                           "urban")
  local_df$quarter_of_diagnosis = relevel(as.factor(local_df$quarter_of_diagnosis), ref =
                                            "Q1 2020")
  
  aa_fit <-
    aareg(
      Surv(time, status) ~ three_category_ruca + factor(gender) + factor(Race) + factor(Ethnicity) + factor(BMI_Group) + age + CCI_INDEX + factor(quarter_of_diagnosis) ,
      data = local_df
    )
  str(aa_fit)
  ...
  plot(aa_fit_plot)
}@transform_pandas(Output(rid = "ri.vector.main.execute.80d572c0-cbdb-4e02-993e-9596d9a9f378"))
all_adjusted_OR <- function() {
  ...
  p <- df %>%
    ...
    annotate(
      ...
      size = 3.5,
      hjust = 0
    ) + ggtitle('Estimated Odds of All-Cause Mortality') +
...
all_log_model_only_rurality <- function(summary_all_for_analysis) {
  ...
  local_df <-
    summary_all_for_analysis %>% dplyr::select(
      three_category_ruca,
      gender,
      age_Group,
      Race,
  ...
  local_df$quarter_of_diagnosis = relevel(as.factor(local_df$quarter_of_diagnosis), ref =
                                            "Q2 2020")
  local_df$three_category_ruca <-
    factor(
      local_df$three_category_ruca,
      levels = c("urban", "urban-adjacent rural", "nonurban-adjacent rural")
    )
  
  m1 <-
    glm(
      in_death_table ~ three_category_ruca,
      data = local_df,
      family = binomial,
      maxit = 100
    )
...
all_n3c_summary <- function(summary_table_for_analysis) {
  local_df <-
    summary_table_for_analysis %>% dplyr::select(
      three_category_ruca,
      gender,
      age_Group,
      Race,
  ...
  table1 <-
    local_df %>% tbl_summary(by = three_category_ruca) %>% add_p()
  table1 <- as_tibble(table1, col_labels = FALSE)
  print(table1)
  return(table1)
}@transform_pandas(Output(rid = "ri.vector.main.execute.7b2101e4-5f60-4cc3-9780-21fd5d2f7933"))
all_unadjusted_OR <- function() {
  ...
  p <-
    ...
    annotate(
      ...
      size = 3.5,
      hjust = 0
    ) + ggtitle('Estimated Odds of All-Cause Mortality in Entire Population') +
    theme(plot.title = element_text(hjust = .5, size = 20))
  plot(p)
  #return(null)
}@transform_pandas(
...
cox_fit <- function(survival_data_hosp) {
  ...
  cox <-
    coxph(
      Surv(time, status) ~ three_category_ruca + factor(gender) + factor(Race) + factor(Ethnicity) + factor(BMI_Group) + age + Q_Score + factor(quarter_of_diagnosis) ,
      data = local_df
    )
...
dta_residuals <- function(summary2_for_analysis) {
  ...
  local_df$Ethnicity = relevel(as.factor(local_df$Ethnicity), ref = "Not Hispanic or Latino")
  local_df$BMI_Group = relevel(as.factor(local_df$BMI_Group), ref = "18.5-24.9")
  local_df$four_category_ruca = relevel(as.factor(local_df$four_category_ruca), ref =
                                          "urban")
  local_df$quarter_of_diagnosis = relevel(as.factor(local_df$quarter_of_diagnosis), ref =
                                            "Q1 2020")
  ...
  m1 <-
    glm(
      in_death_table ~ factor(gender) + factor(Race) + factor(Ethnicity) + factor(BMI_Group) + age + factor(Q_Score_Categories) + factor(quarter_of_diagnosis),
      data = local_df,
      family = binomial,
      maxit = 100
    )
  ...
  table2 <-
    local_df %>% tbl_summary(by = four_category_ruca) %>% add_p()
...
hosp_log_model <- function(summary2_for_analysis) {
  ...
  print(summary2_for_analysis %>% summary())
  local_df <-
    summary2_for_analysis %>% dplyr::select(
      three_category_ruca,
      gender,
      age_Group,
      Race,
  ...
  local_df$three_category_ruca = relevel(as.factor(local_df$three_category_ruca), ref =
                                           "urban")
  local_df$age_Category = relevel(as.factor(local_df$age_Category), ref =
                                    "30-49")
  local_df$quarter_of_diagnosis = relevel(as.factor(local_df$quarter_of_diagnosis), ref =
                                            "Q3 2020")
  local_df$three_category_ruca <-
  ...
  m1 <-
    glm(
      in_death_table ~ factor(gender) + factor(Race) + factor(Ethnicity) + BMI + age + CCI_INDEX + factor(quarter_of_diagnosis) + factor(three_category_ruca),
      data = local_df,
      family = binomial,
      maxit = 100
    )
...
hosp_log_model_bmi <- function(summary2_for_analysis) {
  ...
  m1 <-
    glm(
      in_death_table ~ factor(BMI_Group),
      data = local_df,
      family = binomial,
      maxit = 100
    )
...
hosp_log_model_data_partner_only <-
  function(summary2_for_analysis) {
    library(gtsummary)
    library(rvest)
    library(dplyr)
    print(summary2_for_analysis %>% summary())
    ...
    m1 <-
      glm(
        in_death_table ~ factor(gender) + factor(Race) + factor(Ethnicity) + factor(BMI_Group) + age + CCI_INDEX + factor(quarter_of_diagnosis) + factor(data_partner_id),
        data = local_df,
        family = binomial,
        maxit = 100
      )
...
hosp_log_model_no_BMI <- function(summary2_for_analysis) {
  ...
  m1 <-
    glm(
      in_death_table ~ factor(gender) + factor(Race) + factor(Ethnicity) + factor(BMI_Group) + age + CCI_INDEX + three_category_ruca + factor(quarter_of_diagnosis),
      data = local_df,
      family = binomial,
      maxit = 100
    )
...
hosp_log_model_on_admission <-
  function(summary_for_analysis_on_hospitalization) {
    library(gtsummary)
    library(rvest)
    library(dplyr)
    library(survminer)
    ...
    local_df <-
      summary_for_analysis_on_hospitalization %>% dplyr::select(
        data_partner_id,
        three_category_ruca,
        gender,
        age_Group,
    ...
    m1 <-
      glmer(
        hospitalized ~ factor(gender) + factor(Race) + factor(Ethnicity) + BMI + age + CCI_INDEX + factor(quarter_of_diagnosis) + (1 |
                                                                                                                                     data_partner_id) + factor(three_category_ruca),
        data = local_df,
        na.action = "na.omit",
...
hosp_log_model_on_admission_only_rurality <-
  function(summary_for_analysis_on_hospitalization) {
    library(gtsummary)
    library(rvest)
    library(dplyr)
...
hosp_log_model_only_rurality_[REDACTED] <-
  function(summary2_for_analysis) {
    library(gtsummary)
    library(rvest)
    library(dplyr)
...
hosp_log_model_rural_percentage <- function(summary2_for_analysis) {
  ...
  m1 <-
    glm(
      in_death_table ~ factor(gender) + factor(Race) + factor(Ethnicity) + BMI + age + CCI_INDEX + rural_percentage + factor(quarter_of_diagnosis),
      data = local_df,
      family = binomial,
      maxit = 100
    )
...
hosp_log_model_rurality_only_random_effect <-
  function(summary2_for_analysis) {
    library(gtsummary)
    library(rvest)
    library(dplyr)
    ...
    m1 <-
      glmer(
        in_death_table ~  factor(three_category_ruca) + (1 |
                                                           data_partner_id),
        data = local_df,
        na.action = "na.omit",
        family = binomial(logit)
...
hosp_model_data_partner_random_effect <-
  function(summary2_for_analysis) {
    library(gtsummary)
    library(rvest)
    library(dplyr)
    ...
    m1 <-
      glmer(
        in_death_table ~ factor(gender) + factor(Race) + factor(Ethnicity) + BMI + age + CCI_INDEX + factor(quarter_of_diagnosis) + (1 |
                                                                                                                                       data_partner_id) + factor(three_category_ruca),
        data = local_df,
        na.action = "na.omit",
        family = binomial(logit)
    ...
    table1 <- as_tibble(table1, col_labels = FALSE)
    return(table1)
  }@transform_pandas(Output(rid = "ri.vector.main.execute.ff5170d6-6037-4554-8c2f-6d1bbe951c81"))
hosp_unadjusted_OR <- function() {
  ...
  p <-
    ...
    annotate(
      ...
      size = 3.5,
      hjust = 0
    ) + ggtitle('Estimated Odds of All-Cause Mortality in Hospitalized Population') +
    theme(plot.title = element_text(hjust = .5, size = 20))
  plot(p)
  #return(null)
}@transform_pandas(
...
inpatient_summary <- function(summary2_for_analysis) {
  ...
  local_df <-
    summary2_for_analysis %>% dplyr::select(
      ...
      Dexamethasone,
      Hydrocortisone,
      Prednisone_Methylprednisolone,
      Prednisolone,
      Chloroquine_Hydroxychloroquine
    )
...
inpatient_summary_no_missing_BMI <-
  function(summary2_for_analysis) {
    library(gtsummary)
    library(rvest)
    library(dplyr)
...
km_AG_fit <- function(survival_data_hosp) {
  ...
  local_df$quarter_of_diagnosis = relevel(as.factor(local_df$quarter_of_diagnosis), ref =
                                            "Q2 2020")
  local_df$age_Category <-
    factor(local_df$age_Category,
           levels = c("<18", "18-29", "30-49", "50-64", ">=65"))
  ...
  km_trt_fit <-
    survfit(Surv(time, status) ~ three_category_ruca, data = local_df)
  km_trt_fit_plot <- autoplot(km_trt_fit)
  #plot(km_trt_fit_plot)
  
  km_AG_fit <-
    survfit(Surv(time, status) ~ age_Category, data = local_df)
...
km_AG_fit_copied <- function(survival_data_hosp) {
  ...
  km_trt_fit <-
    survfit(Surv(time, status) ~ quarter_of_diagnosis, data = local_df)
  km_trt_fit_plot <- autoplot(km_trt_fit)
  #plot(km_trt_fit_plot)
  
  km_AG_fit <-
    survfit(Surv(time, status) ~ quarter_of_diagnosis, data = local_df)
  km_trt_fit$panel$layout$strata <-
    c("Q1 2020", "Q1 2021", "Q2 2020", "Q3 2020", "Q4 2020")
...
km_trt_fit <- function(survival_data_hosp) {
  ...
  km_trt_fit <-
    survfit(Surv(time, status) ~ rural_categories, data = local_df)
  
  km_trt_fit$panel$layout$names <-
    c("Urban", "Urban-Adjacent Rural", "Nonurban-Adjacent Rural")
  str(km_trt_fit)
...
km_trt_fit_age <- function(survival_data_hosp) {
  ...
  #km_trt_fit <- survfit(Surv(time, status) ~ three_category_ruca, data=local_df)
  km_trt_fit <-
    survfit(Surv(time, status) ~ age_Group, data = local_df)
...
km_trt_fit_bmi <- function(survival_data_hosp) {
  ...
  local_df$BMI_Group <-
    factor(
      local_df$BMI_Group,
      levels = c("<18.5", "18.5-24.9", "25-29.9", ">30", "Unknown/Missing")
    )
  ...
  #km_trt_fit <- survfit(Surv(time, status) ~ three_category_ruca, data=local_df)
  km_trt_fit <-
    survfit(Surv(time, status) ~ BMI_Group, data = local_df)
  
  km_trt_fit$panel$layout$names <-
    c("<18.5", "18.5-24.9", "25-29.9", ">30", "Unknown/Missing")
  str(km_trt_fit)
...
km_trt_fit_cci <- function(survival_data_hosp) {
  ...
  local_df$quarter_of_diagnosis = relevel(as.factor(local_df$quarter_of_diagnosis), ref =
                                            "Q2 2020")
  local_df$CCI_Categories <-
    factor(local_df$CCI_Categories, levels = c("<1.0", "1.0-2.0", ">2.0"))
  ...
  #km_trt_fit <- survfit(Surv(time, status) ~ three_category_ruca, data=local_df)
  km_trt_fit <-
    survfit(Surv(time, status) ~ CCI_Categories, data = local_df)
...
km_trt_fit_quarter_of_diagnosis <- function(survival_data_hosp) {
  ...
  #local_df$rural_categories=relevel(as.factor(local_df$rural_categories),ref="Urban")
  #local_df$quarter_of_diagnosis=relevel(as.factor(local_df$quarter_of_diagnosis),ref="Q2 2020")
  local_df$quarter_of_diagnosis <-
    factor(
      local_df$quarter_of_diagnosis,
      levels = c("Q1 2020", "Q2 2020", "Q3 2020", "Q4 2020", "Q1 2021")
    )
  ...
  km_trt_fit$panel$layout$names <-
    c("Q1 2020", "Q2 2020", "Q3 2020", "Q4 2020", "Q1 2021")
  str(km_trt_fit)
...
log_all <- function(summary_all_for_analysis) {
  ...
  m1 <-
    glm(
      in_death_table ~ factor(gender) + factor(Race) + factor(Ethnicity) + factor(BMI_Group) + age + CCI_INDEX + factor(three_category_ruca) + factor(quarter_of_diagnosis),
      data = local_df,
      family = binomial,
      maxit = 100
    )
...
log_all_adm_outcome <- function(summary_all_for_analysis) {
  ...
  m1 <-
    glm(
      admission_status ~ factor(gender) + factor(Race) + factor(Ethnicity) + factor(BMI_Group) + age + Q_Score + factor(three_category_ruca) + factor(quarter_of_diagnosis),
      data = local_df,
      family = binomial,
      maxit = 100
    )
  ...
  table1 <- as_tibble(table1, col_labels = FALSE)
  return(table1)
}@transform_pandas(Output(rid = "ri.vector.main.execute.2cc2b17e-415e-44bb-87a1-718f528613d5"))
...
log_rank_surv <- function(survival_data_hosp) {
  ...
  splots[[1]] <- ggsurvplot(
    ...
    palette = c("#404788FF", "#95D840FF"),
    #palette = scale_fill_viridis(option="A"),  # pass the active palette
    ...
    # survival estimates.
    xlab = "Time in Days From Hospital Admission",
  ...
  surv_diff <-
    survdiff(Surv(time, status) ~ irural, data = local_df)
...
m1_rural_vars_copied <- function(dta_residuals) {
  ...
  local_df <-
    dta_residuals %>% dplyr::select(four_category_ruca,
                                    gender,
                                    Race,
                                    Ethnicity,
                                    BMI_Group,
  ...
  tb2 <-
    dta_residuals %>% group_by(four_category_ruca, gender) %>% summarise(rs_m =
                                                                           mean(rs))
  wide <-
    reshape(tb2,
            idvar = c("four_category_ruca"),
            direction = "wide")
  print(wide)
...
m2_rural_copied <- function(dta_residuals) {
  ...
  m2 <-
    glm(
      in_death_table ~ factor(gender) + factor(Race) + factor(Ethnicity) + factor(BMI_Group) + age + factor(Q_Score_Categories) +
        factor(four_category_ruca),
      data = local_df,
      family = binomial,
      maxit = 100
    )
  ...
  return(local_df)
}@transform_pandas(Output(rid = "ri.vector.main.execute.35d85d44-7f19-45df-baec-75704c471409"))
...
m3_rural_int_copied <- function(dta_residuals) {
  ...
  m3_gender <-
    glm(
      in_death_table ~ factor(gender) + factor(Race) + factor(Ethnicity) + factor(BMI_Group) + age + factor(Q_Score_Categories) +
        factor(four_category_ruca) + four_category_ruca * gender,
      data = local_df,
      family = binomial,
      maxit = 100
    )
  m3_Race <-
    glm(
      in_death_table ~ factor(gender) + factor(Race) + factor(Ethnicity) + factor(BMI_Group) + age + factor(Q_Score_Categories) +
        factor(four_category_ruca) + four_category_ruca * Race,
      data = local_df,
      family = binomial,
      maxit = 100
    )
  m3_Q_Score_Categories <-
    glm(
      in_death_table ~ factor(gender) + factor(Race) + factor(Ethnicity) + factor(BMI_Group) + age + factor(Q_Score_Categories) +
        factor(four_category_ruca) + four_category_ruca * Q_Score_Categories,
      data = local_df,
      family = binomial,
      maxit = 100
    )
...
}@transform_pandas(Output(rid = "ri.vector.main.execute.e98b0f75-974f-4155-b87d-e3e04bc6aa85"))
...
margin_log_all <- function(summary_all_for_analysis) {
  ...
  return(margins1)
}@transform_pandas(Output(rid = "ri.vector.main.execute.8de3defa-e180-469d-bf0d-9d010567d33a"))
#not working
  ...
  table12 <- local_df %>% group_by(factor(Race)) %>%
    summarise(mean_rs_isolated = mean(rs[four_category_ruca == "urban"])
              ...
              table13 <- local_df %>% group_by(Ethnicity) %>%
                summarise(
                  mean_rs_isolated = mean(rs[four_category_ruca == "isolated"])
                  ,
                  mean_rs_large_rural = mean(rs[four_category_ruca == "large rural"])
                  ,
                  mean_rs_small_rural = mean(rs[four_category_ruca == "small rural"])
...
resid_in_m_copied <- function(dta_residuals) {
  ...
  varlist <-
    list("Severity_Type", "ECMO", "MACE", "Mechnical_Ventilation")
  for (vvv in varlist) {
...
summary_all_filtered_to_final_analysis <-
  function(summary_for_analysis_on_hospitalization) {
    library(gtsummary)
    library(rvest)
    library(dplyr)
...
survival_admission_on_rurality <-
  function(summary_for_analysis_on_hospitalization) {
    library(survival)
    library(ranger)
    library(ggplot2)
...
survival_final_copied <- function(survival_data_hosp) {
  ...
  ggsurv <- ggsurvplot(
    ...
    # show confidence intervals for
    # point estimates of survival curves.
    palette = c("#404788FF", "#95D840FF", "#FDE725FF"),
    # pass the active palette
    ggtheme      = NULL,
    # disable adding custom survminer theme
    font.x       = NULL,
    # disable adding custom survminer font for the x axis
    font.y       = NULL,
    # disable adding custom survminer font for the y axis
    font.main    = NULL,
    # disable adding custom survminer font for the title
    font.submain = NULL,
    # disable adding custom survminer font for the subtitle
    font.caption = NULL,
    # disable adding custom survminer font for the caption
    xlim = c(0, 30),
    # present narrower X axis, but not affect
    # survival estimates.
    xlab = "Time in days",
...
survival_final_copied_zoomed <- function(survival_data_hosp) {
  ...
  splots[[1]] <- ggsurvplot(
    ...
    # show confidence intervals for
    # point estimates of survival curves.
    palette = c("#DCE319FF", "#440154FF", "#39568CFF"),
    #palette = scale_fill_viridis(option="A"),  # pass the active palette
    #ggtheme      = NULL, # disable adding custom survminer theme
    #font.x       = NULL, # disable adding custom survminer font for the x axis
    #font.y       = NULL, # disable adding custom survminer font for the y axis
...
tb_resid_copied <- function(dta_residuals) {
  ...
  m1 <-
    glm(
      rs ~ factor(gender) + factor(Race) + factor(Ethnicity) + factor(BMI_Group) + factor(age_Group) + factor(Q_Score_Categories) + i_rural ,
      data = local_df,
      maxit = 100
    )
  print(summary(m1))
  ...
  local_df$Race =  as.factor(local_df$Race)
  m2 <-
    glm(rs ~   i_rural * Q_Score_Categories,
        data = local_df,
        maxit = 100)
  ...
  table2 <-
    local_df %>% tbl_summary(by = Q_Score_Categories) %>% add_p()
  table2 <- as_tibble(table2, col_labels = FALSE)
  #print(table2)
  ...
  return (NULL)
  #return (local_df2)
}@transform_pandas(Output(rid = "ri.vector.main.execute.c1b41359-bbc0-4ec3-900a-a7cf260427f3"))
...
unnamed_9 <- function(hospitalized_survived) {
  ...
  local_df <-
    hospitalized_survived %>% dplyr::select(
      three_category_ruca,
      gender,
      age_Group,
      Race,
