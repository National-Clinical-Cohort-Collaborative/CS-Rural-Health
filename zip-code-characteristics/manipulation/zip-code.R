...
ds2 <-
  ...
  dplyr::summarize(
    ...
    count_within_20   = sum(distance_from_zip_code_to_city_in_miles <=  20L),
    count_within_60   = sum(distance_from_zip_code_to_city_in_miles <=  60L),
    count_within_100  = sum(distance_from_zip_code_to_city_in_miles <= 100L),
  ) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(
